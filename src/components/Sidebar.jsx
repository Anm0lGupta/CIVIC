...
import { Home, Map, BarChart3, User, PlusCircle, Shield, Wifi } from 'lucide-react';
    ...
    <aside className="hidden lg:flex lg:flex-col lg:w-64 bg-slate-900 border-r border-slate-800 h-screen sticky top-0 flex-shrink-0">
      ...
      <nav className="flex-1 p-4 overflow-y-auto">
              ...
              <li key={item.path}>
                <NavLink to={item.path} end={item.path === '/dashboard'}
                  className={({ isActive }) => `flex items-center gap-3 px-4 py-3 rounded-lg transition-all duration-200 ${
                    isActive ? 'bg-orange-500 text-white shadow-lg shadow-orange-500/20' : 'text-slate-300 hover:bg-slate-800 hover:text-white'
                  }`}>
                  <Icon className="w-5 h-5 flex-shrink-0" />
                  <span className="font-medium flex-1">{item.label}</span>
                  {item.badge && (
                    <span className="text-xs bg-violet-500 text-white px-1.5 py-0.5 rounded font-bold">{item.badge}</span>
                  )}
                </NavLink>
              </li>
            );
        ...
        <div className="mt-4 pt-4 border-t border-slate-800">
          <NavLink to="/admin" className={({ isActive }) => `flex items-center gap-3 px-4 py-3 rounded-lg transition-all duration-200 ${
            isActive ? 'bg-orange-500 text-white' : 'text-slate-500 hover:bg-slate-800 hover:text-slate-300'
          }`}>
